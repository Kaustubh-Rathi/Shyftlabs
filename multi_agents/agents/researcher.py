...
from .utils.vector_index import query_global_vector_store
...
class ResearchAgent:
    ...
    async def run_subtopic_research(self, parent_query: str, subtopic: str, verbose: bool = True, source="web", headers=None):
        retrieved_chunks = query_global_vector_store(subtopic, k=3, threshold=1.0)
        if len(retrieved_chunks) >= 2:
            report = "\n\n".join([chunk["text"] for chunk in retrieved_chunks])
            if self.websocket and self.stream_output:
                await self.stream_output("logs", "reusing_data", f"Reusing existing data for subtopic: {subtopic}", self.websocket)
            else:
                print_agent_output(f"Reusing existing data for subtopic: {subtopic}", agent="RESEARCHER")
        else:
            try:
